...
/**
 * Map function f onto in_list0 and in_list1 placing results in out_list.
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  // TODO
  int len_0 = in_list0->len;
  int len_1 = in_list1->len;
  if (len_0 < len_1) {
    for (int i=0; i < len_0; i++) {
      element_t e0 = list_get(in_list0, i);
      element_t e1 = list_get(in_list1, i);
      f(&e0, e0, e1);
      list_append(out_list, e0);
    }
  }else {
...
/**
 * Foldl in_list using function f placing result in out_element_p.
 * For f(out,in):
...
void list_foldl (void (*f) (element_t*, element_t, element_t), element_t* out_element_p,  struct list* in_list) {
  ...
  for (int i=0; i < in_list->len; i++) {
    element_t e = list_get(in_list, i);
    f(out_element_p, e, *out_element_p);
  }
}
